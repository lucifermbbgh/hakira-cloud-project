...
> 阶段：会计账簿数据处理集成系统（hakira-cloud-project）——基础设施部署 + 核心链路冒烟测试
> 日期：2026-08-13
> 分支：`release-202608-remake`
...
将 hakira-cloud-project 重构为会计账簿数据处理集成系统（复式记账）后，完成**基础设施部署**与**核心链路冒烟测试**，使以下链路真正跑通：
...
| Nacos | `nacos/nacos-server:v3.2.3` | `hakira_ledger_nacos` | 8848/9848 | 注册中心+配置中心，内置存储模式 |
...
- 登录凭据：`admin / admin123`（数据库 `hakira_auth.hakira_user` 表，NoOpPasswordEncoder 明文匹配）
...
| 3 | 端口 9000/9010/9020/9030 监听 | 全部监听 | ✅ |
...
- **结果**：✅ Nacos 启动成功（注册中心/配置中心功能正常，与 MySQL 持久化解耦）。
...
- **现象**：stock 的 `snapshot`/`movements` 接口返回 500，报 `Name for argument of type [java.lang.String] not specified, and parameter name information not found`。
...
| # | 问题 | 影响 | 建议 |
|---|------|------|------|
| 1 | Nacos 连 MySQL 未解决（当前内置存储） | Nacos 配置数据不持久化（重启丢失） | 后续单独研究 Nacos 3.x 外部数据源配置 |
| 2 | Nacos 3.x 认证 403 刷屏 | 日志噪音，不影响服务注册（走 gRPC） | 配置正确的 Nacos 3.x 认证凭据或调整 discovery 配置 |
| 3 | `NoOpPasswordEncoder` 明文密码 | 仅冒烟测试用，有安全警告 | 生产环境切换 BCrypt + 密码哈希 |
| 4 | 借贷不平衡/库存不足返回 HTTP 500 | 无统一业务错误码 | 增加全局异常处理器（@RestControllerAdvice） |
| 5 | stock/entry 内存存储 | 重启后数据丢失 | 后续接入 `hakira_ledger` 数据库持久化 |
...
| **v7（本次）** | `docker-compose-infra.yml` | **账本专用重构**：`mysql:8.0` + `nacos:v3.2.3`，容器名 `hakira_ledger` 前缀，端口 3307/8848/9848 |
