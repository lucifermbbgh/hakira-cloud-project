...
public interface StockSnapshotMapper {
    ...
    @Insert("INSERT INTO stock_snapshot(item_code, item_name, current_quantity, total_cost, weighted_avg_cost, unit, status, version) " +
            "VALUES(#{itemCode}, #{itemName}, #{currentQuantity}, #{totalCost}, #{weightedAvgCost}, #{unit}, 'ACTIVE', 0)")
    ...
    @Update("UPDATE stock_snapshot SET current_quantity = #{currentQuantity}, total_cost = #{totalCost}, " +
            "weighted_avg_cost = #{weightedAvgCost}, item_name = #{itemName}, unit = #{unit}, version = version + 1 " +
