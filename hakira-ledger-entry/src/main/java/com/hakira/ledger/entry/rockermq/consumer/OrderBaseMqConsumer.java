package com.hakira.ledger.entry.rockermq.consumer;
 ...
 * @BelongsPackage: com.hakira.ledger.entry.rockermq.consumer
 * @Author: hakiraKafka
...
public class OrderBaseMqConsumer {
    ...
    public static DefaultMQPushConsumer initConsumer() {
        // 创建消费者
        DefaultMQPushConsumer consumer = new DefaultMQPushConsumer("hakira-ledger-entry-consumer");
        // 指定NameServer地址
    ...
    public static void main(String[] args) {
        ...
        try {
            // 订阅消息，根据正则表达式过滤
            consumer.subscribe("hakira-ledger-entry-topic", "*");
            // 订阅消息，根据SQL表达式过滤
            consumer.subscribe("hakira-ledger-entry-topic", MessageSelector.bySql("a between 0 and 3"));
            // 订阅消息，根据Tag过滤
            consumer.subscribe("hakira-ledger-entry-topic", MessageSelector.byTag("tagA || tagB"));
            // 取消订阅消息
            consumer.unsubscribe("hakira-ledger-entry-default-topic");
            // 获取消费者对主题分配了哪些消息队列
            Set<MessageQueue> messageQueues = consumer.fetchSubscribeMessageQueues("hakira-ledger-entry-topic");
