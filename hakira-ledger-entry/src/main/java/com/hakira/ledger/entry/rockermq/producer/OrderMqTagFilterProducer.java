...
public class OrderMqTagFilterProducer {
    public static void main(String[] args) {
        // 创建生产者
        DefaultMQProducer producer = new DefaultMQProducer("hakira-ledger-entry-tagFilter-producer");
        // 设置NameServer地址
        ...
        try {
            ...
            for (int i = 0; i < 10; i++) {
                // 创建消息
                Message message = new Message("hakira-ledger-entry-tagFilter-topic",
                        tags[i % tags.length],
                        "hakira-ledger-entry-tagFilter-key",
                        ("hakira-ledger-entry-tagFilter-body" + i).getBytes(RemotingHelper.DEFAULT_CHARSET));
                // 设置消息属性
